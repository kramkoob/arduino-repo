...
#define STEP_RATIO 4 // microstepping ratio
...
ISR(TIMER2_COMPA_vect) {
  for (uint8_t stepper_id = 0; stepper_id < NUM_STEPPERS; stepper_id++) {
    if(stepper_state[stepper_id]){
      if(++stepper_step_count[stepper_id] >= stepper_step_target[stepper_id]){
...
void setdir(uint8_t stepper_id, boolean direction){
  if(direction == true){
...
void setRPM(uint8_t stepper_id, uint8_t rpm){
  ...
  if(rpm != 0){
    stepper_state[stepper_id] = true;
    stepper_step_target = uint8_t(1000000 / (float(rpm) / 60.0 * STEP_FULL * STEP_RATIO) / 4 / TIMER_OCR2A);
  }else{
    stepper_state[stepper_id] = false;
    stepper_count[stepper_id] = 0;
  }
}
void loop() {
